...
A REST API (FastAPI + SQLAlchemy) for a small clinic with 5 doctors and 30 minute
appointment slots, plus a React frontend that consumes it. Built for the
...
| **Hosting** | [Render](https://render.com). API, frontend, and PostgreSQL all run on Render's free tier |
...
> first and wait for it to respond (about 30 to 60 seconds on a cold start).
> That wakes the backend so the frontend's requests succeed instead of timing
> out. Swagger docs are at [`/docs`](https://clinic-booking-api-9ttn.onrender.com/docs)
> on the same API URL.
...
| Section 3: Deployment and CI/CD | [Section 3](#section-3-deployment-and-cicd) below |
| Section 4: AI reflection | [`AI_REFLECTION.md`](./AI_REFLECTION.md) |
| Bonus: 1 hour minimum notice | [Key decisions](#key-decisions-and-trade-offs) and [endpoint table](#endpoints) |
| Bonus: patient appointment history | `GET /patients/{id}/appointments` in [endpoint table](#endpoints) |
| Bonus: concurrency and double booking safety | [Key decisions](#key-decisions-and-trade-offs) and `tests/test_concurrency.py` |
...
flowchart TD
    ...
    subgraph Render["Render, Free Tier"]
        direction TB
        FE["Frontend, Static Site<br/>clinic-booking-frontend<br/>.onrender.com"]
        API["Backend, Web Service<br/>clinic-booking-api<br/>.onrender.com<br/>FastAPI + Uvicorn"]
        DB[("PostgreSQL<br/>clinic-booking-db")]
...
Five doctors, each with fixed weekly working hours, work in 30 minute slots.
...
Two constraints matter more than the columns. `WorkingHours` has
`UNIQUE(doctor_id, day_of_week)`, one schedule row per doctor per weekday.
`Appointment` has a partial unique index,
`UNIQUE(doctor_id, slot_start) WHERE status = 'booked'`, which is the real
...
flowchart TD
    ...
    R["FastAPI Routers<br/>appointments / doctors / patients<br/>HTTP to service translation only"] --> S

    S["BookingService<br/>all business rules and concurrency handling"] --> Repo

    Repo["Repositories<br/>Appointment / Doctor / Patient<br/>queries only, no business logic"] --> M

    M["SQLAlchemy Models"] --> DB[("PostgreSQL (prod)<br/>SQLite (local and tests)")]
...
Every error response follows this `{error, message}` shape no matter which
layer raised it, so the frontend and any API client have exactly one
error parsing path.
...
`WorkingHours` at read time and subtracts what's already `booked`. One
source of truth, no background job generating future slots. The trade-off is
more computation per request. At five doctors that's free. At hundreds I'd
add a cached slots table refreshed on a schedule, but building that now would
solve a problem this clinic doesn't have.
...
fast, friendly `409`, but the loser of a real race is rejected by the
database with an `IntegrityError` that the service turns into a clean `409`.
This holds across multiple app instances behind a load balancer. See
`app/services/booking_service.py` and `tests/test_concurrency.py`, which
races two real threads for the same slot and asserts exactly one wins.

**Reschedule updates the same row rather than cancel and recreate.** Changing
`slot_start` and `slot_end` in a single `UPDATE` makes freeing the old slot
and claiming the new one atomic by construction. There is no window where
both slots are briefly free or briefly double claimed, and the same index
still guards the new slot.

**Cancelled appointments don't block rebooking.** Because the index is
partial, a cancelled row leaves nothing blocking that doctor and slot pair.
The slot is immediately bookable again.

**All datetimes are UTC**, converted at the API boundary. Clients send
timezone aware ISO 8601 values. Validation, slot math, and storage are all
UTC. A custom `UTCDateTime` type (`app/core/types.py`) normalises to naive
UTC on write and re-attaches `tzinfo=UTC` on read, because SQLite silently
drops timezone info even on a `DateTime(timezone=True)` column. That had made
a freshly booked slot fail to match the datetime availability was excluding.
Behaviour is now identical on SQLite (dev and tests) and Postgres (prod).

**The 1 hour minimum notice (bonus) applies to bookings and reschedules**,
and shows up in `GET availability` too. A slot 20 minutes from now isn't
listed, since booking it would be rejected anyway. Showing it and then
refusing would be dishonest to the client.
...
- A doctor with no `WorkingHours` row for a weekday is treated as not working
  ...
  error. The common real world case is simply that the doctor doesn't work
  weekends.
...
- Cancellation requires a non-empty reason, 1 to 500 characters, per "cancel
  with a reason."
...
|---|---|---|---|
| `POST` | `/appointments` | Book a slot. Validated against working hours, past times, the 30 minute grid, 1 hour notice, and existing bookings | 201 / 400 / 404 / 409 / 422 |
...
Full request and response schemas are in Swagger at `/docs` once running.
Kept out of this README so the two don't drift out of sync.

### Error handling strategy

All four mutating and validating endpoints share one exception to HTTP
mapping (see [Section 1](#section-1-system-design)).
...
business rule validation that depends on doctor or schedule state.

### Running locally

Full step by step instructions (installs, Alembic, DB inspection, frontend,
Render) are in [`SETUP.md`](./SETUP.md). The short version:
...
python seed.py                # 5 doctors Mon to Fri 09:00 to 17:00 UTC, 2 patients
uvicorn app.main:app --reload --port 8000

# Frontend (repo root, separate terminal)
npm install && cp .env.example .env && npm run dev   # http://localhost:8080
```
...
doctor or patient, notice window), cancel and reschedule state transitions,
...
static frontend, and managed PostgreSQL in one dashboard, with zero config
...
2. **On merge to `main`:** re-runs those checks, then posts to Render's
   deploy hook (`DEPLOY_HOOK_URL` secret) for the `clinic-booking-api`
   service. Render then pulls the new commit, rebuilds, and redeploys.
   Keeping the deploy behind a hook rather than provider specific Actions
   steps means no Render credentials or service IDs live in this repo, only
   the one secret URL.
3. **Migrations:** Render runs `alembic upgrade head` as the service's
   release command before the new instance takes traffic, so a bad migration
   blocks the deploy instead of shipping half migrated.
...
**Secrets and environment:** `DATABASE_URL` and `ALLOWED_ORIGINS` are read
from the environment (`app/core/config.py`) and never committed. Locally
...
patient, and date pickers, a live availability grid, booking, cancel with
reason, and reschedule. Times render in UTC to match the API contract
exactly, and error toasts reuse the API's own `error` code and `message`
rather than inventing wording. It talks to the API purely over HTTP. No
